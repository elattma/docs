...
Below you will see an example of how to build a LangChain agent that uses the Mimo API in two different tools to write contextualize business emails. The tools at the agent's disposal include:

- Knowledge Tool - allows the agent to query for any context about the enterprise necessary to write the email
- Directory Tool - allows the agent to look up a recipient's email address

<Info>
  This is a very lightweight guide that assumes you have prior experience building AI agents with LangChain.
</Info>
...
HENRY WRITE THIS ONE LINER THANKS
